...
try:
    ...
    from models.NodeEncoder import NodeEncoder_QF_AddPlanrows # 导入新的 Encoder
...
class QueryFormerToPyGDataset(Dataset):
    def __init__(self, df, encoding, hist_file, table_sample, cost_norm, cache_name=None):
        ...
        for idx in tqdm(range(len(self.df)), desc="Processing"):
            ...
            # === 新增: 获取 Plan Rows 并拼接到 x ===
            plan_rows = float(json.loads(json_str)['Plan']['Plan Rows'])
            plan_rows_log = np.log1p(plan_rows)
            # 扩展 plan_rows 到每个节点? 不，Plan Rows 是每个节点都有的属性
            # qf_dataset 提取的特征只包含了 QF 定义的 1165 维
            # 我们需要重新遍历一遍树，把 Plan Rows 提取出来，或者修改 QF 的 traversePlan (太麻烦)
            # 简单做法：这里我们只能拿到根节点的 Plan Rows，但树的每个节点都有自己的 Plan Rows。
            # 既然无法轻易修改 QF 的 dataset 代码，我们只能在这里做一个妥协：
            # 如果 node_dict['features'] 里的顺序和 plan 遍历顺序一致（通常是 DFS），我们可以并行遍历 json plan 提取 rows
            
            # 重新遍历提取 Plan Rows (DFS)
            def extract_rows(node, rows_list):
                 rows = float(node['Plan Rows'])
                 rows_list.append(np.log1p(rows))
                 if 'Plans' in node:
                     for child in node['Plans']:
                         extract_rows(child, rows_list)
            
            rows_list = []
            extract_rows(plan, rows_list)
            
            # x 是 Tensor [Num_Nodes, 1165]
            # rows_tensor 是 Tensor [Num_Nodes, 1]
            rows_tensor = torch.tensor(rows_list, dtype=torch.float).view(-1, 1)
            
            # 拼接
            if x.size(0) == rows_tensor.size(0):
                x = torch.cat([x, rows_tensor], dim=1) # [N, 1166]
            else:
                # 理论上应该相等，如果不等说明 QF 的遍历顺序或过滤逻辑(如过滤非 Table 节点?) 不一致
                # QF 会保留所有节点吗？ 看 QF 代码 PlanTreeDataset
                # 是的，通常是保留的。如果报错，说明对齐有问题。
                pass
